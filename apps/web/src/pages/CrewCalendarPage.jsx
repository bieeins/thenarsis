...
const CrewCalendarPage = () => {
  ...
  const loadAssignments = useCallback(async () => {
    ...
    try {
      ...
      console.error('[CrewCalendar] Failed to load assignments:', err);
  ...
  const eventsByDate = useMemo(() => {
    return filteredEvents.reduce((acc, event) => {
      ...
      try {
        ...
        console.warn(`Error parsing date for assignment ${event.assignment_id}`);
      }
      return acc;
    }, {});
  }, [filteredEvents]);
  ...
  const handleEventClick = (event) => {
    navigate(`/crew/events/${event.assignment_id}`);
  };

  return (
    <>
      <Helmet>
        <title>Crew Calendar - Thenarsis</title>
      </Helmet>
        ...
        <div className="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
            <div>
              <div className="flex items-center text-sm text-muted-foreground mb-2">
                ...
                <span className="mx-2">/</span>
                <span className="text-foreground font-medium">Crew Calendar</span>
              </div>
              <h1 className="text-3xl font-extrabold tracking-tight flex items-center gap-3 text-balance">
                <CalendarIcon className="w-8 h-8 text-primary" />
                Crew Calendar
              </h1>
              <p className="text-muted-foreground mt-1">View and manage your assigned events across the month</p>
            </div>
          </div>
          ...
          {loading ? (
            ...
            <div className="bg-card border rounded-xl p-16 shadow-sm flex flex-col items-center justify-center text-center h-[600px]">
              ...
              <h3 className="text-2xl font-bold text-foreground mb-2">No assignments scheduled</h3>
              <p className="text-muted-foreground max-w-md">
                You have no assigned events for {format(currentDate, 'MMMM yyyy')}. When an owner assigns you to an event, it will appear here in real-time.
              </p>
            </div>
          ) : !error ? (
            <CalendarGrid 
