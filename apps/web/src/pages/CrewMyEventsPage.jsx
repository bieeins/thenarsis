...
import CrewEventCard from '@/components/CrewEventCard.jsx';
...
const CrewMyEventsPageContent = () => {
  const { events, loading, error, refetch } = useCrewEvents();
  ...
  useEffect(() => {
    ...
    console.log(`(1) Component Mount/Update`);
    console.log(`(2) useCrewEvents Hook Data Received ->`, { 
      loading, 
      error, 
      eventsIsArray: Array.isArray(events),
      eventsCount: Array.isArray(events) ? events.length : 'invalid' 
    });
    if (error) {
      console.error(`(5) ❌ Error detected in hook data:`, error);
    }
    console.groupEnd();
  ...
  const filteredAndSortedEvents = useMemo(() => {
    console.group(`[CrewMyEventsPage] (3) Filter & Search State Change`);
    console.log(`Applying filters: Query="${searchQuery}", Status="${statusFilter}", Date="${dateFilter}", Package="${packageFilter}", Sort="${sortBy}"`);
    
    if (!Array.isArray(events)) {
      console.log(`Events is not a valid array. Skipping filtering.`);
      console.groupEnd();
      return [];
    }
    
    let result = [...events];
    result = result.filter(e => e !== null && e !== undefined);
  ...
  if (!Array.isArray(events) || events.length === 0) {
    if (!loading && !error) {
      console.log(`[CrewMyEventsPage] 📭 Empty state detected: No events assigned.`);
    }
  } else if (filteredAndSortedEvents.length === 0) {
    if (!loading && !error) {
      console.log(`[CrewMyEventsPage] 📭 Empty state detected: Filter yielded zero results.`);
    }
  }
    ...
    <div className="min-h-screen bg-[#F8FAFC] dark:bg-slate-950 py-8">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <nav className="flex items-center space-x-2 text-sm text-muted-foreground mb-6">
          ...
          <span className="text-foreground font-medium">My Events</span>
        </nav>

        <div className="mb-8 relative">
          <div className="absolute -left-4 top-0 bottom-0 w-1.5 bg-[hsl(var(--accent-yellow))] rounded-r-full hidden sm:block"></div>
          <h1 className="text-3xl font-extrabold tracking-tight text-foreground">My Events</h1>
        ...
        {loading ? (
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
            {[1, 2, 3, 4, 5, 6].map(i => (
              <div key={i} className="border border-border/50 rounded-2xl p-6 h-[380px] flex flex-col bg-card shadow-sm border-l-4 border-l-muted">
          ...
          <div className="text-center py-20 bg-card rounded-2xl border border-dashed border-destructive/50 shadow-sm">
            <AlertCircle className="w-12 h-12 text-destructive mx-auto mb-4" />
            <h3 className="text-xl font-bold mb-2">Error Loading Events</h3>
            <p className="text-muted-foreground mb-6 max-w-md mx-auto">{error}</p>
            <Button onClick={refetch} className="bg-[hsl(var(--accent-yellow))] hover:bg-[hsl(var(--accent-yellow-hover))] text-black">
          ...
          <div className="text-center py-24 bg-card rounded-2xl border border-dashed border-border shadow-sm flex flex-col items-center">
            <div className="w-20 h-20 bg-[hsl(var(--accent-yellow))]/10 rounded-full flex items-center justify-center mb-6">
              <CalendarDays className="w-10 h-10 text-[hsl(var(--accent-yellow-active))]" />
            </div>
            <h3 className="text-2xl font-bold text-foreground mb-2">No events assigned yet</h3>
            <p className="text-muted-foreground max-w-md mb-6">
              You haven't been assigned to any valid fieldwork. Once an owner assigns you to an event, it will appear here.
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
            {filteredAndSortedEvents.map(event => (
              <CrewEventCard key={event.crew_assignment_id || Math.random()} event={event} />
            ))}
          </div>
        )}
...
const CrewMyEventsPage = () => {
  return (
    <>
      <Helmet>
        <title>My Events - Thenarsis Crew</title>
      </Helmet>
      <ErrorBoundary>
        <CrewMyEventsPageContent />
      </ErrorBoundary>
    </>
  );
};

export default CrewMyEventsPage;